...
const PlayersStructure = () => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 top-0 z-10 grid grid-rows-4 sm:py-2 md:pb-8 md:pt-0">
      ...
      <div className="mx-0 flex justify-center gap-0.5 xs:mx-10 xs:justify-around sm:gap-1 md:mx-24 md:gap-8">
        {DEF.map((player) => (
          <Player key={player.id} player={player} />
        ))}
      </div>
      <div className="mx-0 flex justify-around gap-0.5 xs:mx-10 xs:justify-between sm:gap-1 md:mx-24 md:gap-8">
        {MID.map((player) => (
          <Player key={player.id} player={player} />
        ))}
      </div>
