...
const PlayersStructure = ({ allowDelete = true }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 top-0 z-10 grid grid-rows-4 pt-1 xs:pt-2 md:pt-4 sm:py-2 md:pb-8">
      ...
      <div
        className={`flex items-start justify-evenly gap-0.5 px-10 sm:gap-0.5 xl:gap-0 xl:px-14`}
      >
        {DEF.map((player) => (
          <Player deletePlayer={allowDelete} key={player.id} player={player} />
        ))}
