...
const PlayersStructure = () => {
  ...
  const handleOnDrop = (e) => {
    ...
    // dispatch(deletePlayerById(player_id))
  }
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 top-0 z-10 grid grid-rows-4 sm:py-2 md:pb-8 md:pt-0">
      ...
      <div
        ...
        className={`flex items-center justify-center gap-0.5 px-0 xs:justify-around xs:px-10 sm:gap-1 md:gap-8 md:px-16 lg:px-24`}
      >
        {DEF.map((player) => (
          <Player key={player.id} player={player} />
        ))}
